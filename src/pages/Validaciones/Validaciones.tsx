...
// Mapa estático de nombres de estudiantes (mock)
const nombreEstudianteMap: Record<string, string> = {
    '1': 'Admin Demo',
    '2': 'Estudiante Ejemplo',
};
...
const ValidacionesPage: React.FC = () => {
    ...
    // Lista única de IDs de estudiantes con registros
    const estudiantesUnicos = Array.from(
        new Set(registros.map(r => r.estudianteId))
    );
                    ...
                    <tbody>
                        {registrosFiltrados.map(reg => (
                            <tr key={reg.id} className="border-b hover:bg-gray-50">
                                <td className="px-4 py-2">{nombreEstudianteMap[reg.estudianteId] ?? reg.estudianteId}</td>
