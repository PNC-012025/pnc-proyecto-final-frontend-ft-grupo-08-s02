...
const RegistrosPage: React.FC = () => {
  ...
  // Información para PDF y formulario:
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {canFilter && (
        <div className="bg-white p-4 rounded shadow-md space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              <label className="text-sm font-medium">Proyecto/Actividad</label>
              <input
                type="text"
                placeholder="Proyecto o actividad"
                value={info.proyecto}
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={e => setInfo({ ...info, institucion: e.target.value })}
                className="w-full border rounded px-3 py-2"
              />
            </div>
            <div className="flex flex-col">
      ...
      {/* Zona de informacion del estudinate */}
      <div ref={printableRef} className="bg-white p-6 rounded-2xl shadow-lg space-y-6">
        {canFilter && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 divide-y md:divide-y-0 md:divide-x divide-gray-200">
            <div className="space-y-2 pr-4">
              <p><span className="font-bold">Nombre del estudiante:</span> {info.nombre}</p>
              <p><span className="font-bold">Carrera:</span> {info.carrera}</p>
              <p><span className="font-bold">Carnet:</span> {info.carnet}</p>
              <p><span className="font-bold">Teléfono:</span> {info.telefono || '-'}</p>
            </div>
            <div className="space-y-2 pl-4">
              <p><span className="font-bold">Proyecto/Actividad:</span> {info.proyecto || '-'}</p>
              <p><span className="font-bold">Responsable:</span> {info.institucion || '-'}</p>
              <p><span className="font-bold">Fecha inicio:</span> {info.inicio || '-'}</p>
              <p><span className="font-bold">Materia:</span> {materias.find(m => m.id === materiaSel)?.nombre || '—'}</p>
            </div>
          </div>
        )}
